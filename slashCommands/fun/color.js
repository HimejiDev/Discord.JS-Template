...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const getButtons = (toggle = false, choice) => {
      const row = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder()
          ...
          .setCustomId("blue")
          .setStyle(
            toggle == true && choice == "blue" ? "Secondary" : "Primary"
          )
          .setDisabled(toggle),
    ...
    interaction
      .reply({ embeds: [embed], components: [getButtons()] })
      .then((m) => {
        ...
        collector.on("collect", async (i) => {
          ...
          m.interaction.editReply({
            components: [getButtons(true, i.customId)],
          });
          if (i.customId === "exit") return collector.stop();

          return i.followUp(
            `${i.user}, You chose **${
              i.customId.charAt(0).toUpperCase() + i.customId.slice(1)
            } :${i.customId}_circle:**!`
          );
        });
